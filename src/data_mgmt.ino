...
void logging()
{
  ...
  if (!conditioning_duration && printTime != "")
  {
    ...
    if (logFilePath == "/LOG_.csv")
      return;

    std::ostringstream airLog;
    
    uint32_t now = xTaskGetTickCount(); // NEW untested
    uint32_t milliseconds = now % 1000;
    airLog << printTime << "." << milliseconds << ", ";
    ...
    if (!LittleFS.begin())
      return; // Ensure file system initialization succeeds
  ...
  while (file)
  {
    ...
    if (file.isDirectory())
    {
      ...
      if (levels == 3)
        fsString += "<br><td>&nbsp;</td>";
      if (levels == 2)
        fsString += "<td>&nbsp;</td><td>&nbsp;</td>";
      if (levels == 1)
        fsString += "<td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>";
